...
class UserWebSocketControllerTest {
    ...
    void findUser_shouldReturnWebSocketEvent_whenUserExists() {
        ...
        given(userWebSocketMapper.toWebSocketEvent(user)).willReturn(event);

        UserWebSocketEvent result = controller.findUser(id);

        assertThat(result).isEqualTo(event);
        then(findUserUseCase).should().execute(id);
        then(userWebSocketMapper).should().toWebSocketEvent(user);
    }
    ...
    void listUsers_shouldReturnWebSocketEventList_whenUsersExist() {
        ...
        given(userWebSocketMapper.toWebSocketEventList(users)).willReturn(events);
        ...
        then(userWebSocketMapper).should().toWebSocketEventList(users);
    }
}
